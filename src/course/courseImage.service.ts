...
export class CourseImageService {
  ...
  async generate(stickers: Types.ObjectId[], createCourseImageInput: CreateCourseImageInput): Promise<String> {
    ...
    const stickerImageUrls: String[] = await this.stickerService.getImageUrls(stickers);

    const coords: [Number, Number][] = spots.map(s => s.location['coordinates']);
  ...
  async getPolyline(coords: [Number, Number][]): Promise<String> {
    ...
    return Axios.get(pathUrl, {
      ...
      if (routes.length < 1) throw new MapBoxNoRoutesException(routes);
