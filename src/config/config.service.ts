...
export class AppConfigService {
  ...
  async getDB(): Promise<string> {
    ...
    if (level === 'sandbox') {
      return `mongodb://${this.configService.get('app.MONGO_USER')}:${this.configService.get(
        'app.MONGO_PWD',
      )}@${this.configService.get('app.MONGO_IP')}:${this.configService.get('app.MONGO_PORT')}/${this.configService.get(
        'app.MONGO_DB_NAME',
      )}`;
    }
    ...
    if (level === 'production') {
      return `mongodb+srv://${this.configService.get('app.MONGO_USER')}:${this.configService.get(
        'app.MONGO_PWD',
      )}@${this.configService.get('app.MONGO_IP')}/${this.configService.get(
        'app.MONGO_DB_NAME',
